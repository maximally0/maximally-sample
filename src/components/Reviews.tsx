...
const ReviewCard = ({
  ...
  return <div className="bg-gradient-to-b from-white to-cyan-50 rounded-xl p-8 shadow-md border border-cyan-100 hover:shadow-lg transition-all">
      <div className="flex mb-4">
        {Array.from({
        length: 5
      }).map((_, i) => <Star key={i} className={cn("w-5 h-5", i < rating ? "text-amber-400 fill-amber-400" : "text-gray-300")} />)}
      </div>
      ...
      <div className="flex justify-between items-end">
        ...
        {verified && <span className="text-xs bg-teal-50 text-teal-700 px-2 py-1 rounded-full">
            Verified Purchase
          </span>}
      </div>
    </div>;
...
const Reviews = () => {
  ...
  return <section id="reviews" className="section-padding bg-gradient-to-b from-sky-50 to-cyan-50/30" ref={sectionRef}>
      <div className="maximally-container">
        ...
        <div className="relative">
          <div className="overflow-hidden" ref={carouselRef}>
            <div className="hidden md:grid md:grid-cols-3 gap-6">
              {getVisibleReviews().map((review, index) => <ReviewCard key={index} {...review} />)}
            </div>
          ...
          {/* Navigation Arrows */}
          <button onClick={prev} className="absolute top-1/2 -left-4 md:-left-8 transform -translate-y-1/2 w-10 h-10 bg-white rounded-full shadow-md flex items-center justify-center text-cyan-700 hover:text-cyan-900 transition-colors focus:outline-none" aria-label="Previous review">
            <ChevronLeft className="w-6 h-6" />
          </button>
          
          <button onClick={next} className="absolute top-1/2 -right-4 md:-right-8 transform -translate-y-1/2 w-10 h-10 bg-white rounded-full shadow-md flex items-center justify-center text-cyan-700 hover:text-cyan-900 transition-colors focus:outline-none" aria-label="Next review">
        ...
        <div className="flex justify-center mt-8 space-x-2">
          {Array.from({
          length: Math.ceil(reviewData.length / reviewsPerView)
        }).map((_, i) => <button key={i} onClick={() => setActiveIndex(i)} className={cn("w-2.5 h-2.5 rounded-full transition-all duration-300", activeIndex === i ? "bg-cyan-600 w-8" : "bg-cyan-200")} aria-label={`Go to review page ${i + 1}`} />)}
        </div>
